...
plot_factor_scores <- function(mymfa, d1 = 1, d2 = 2) {
  if (ncol(mymfa$common_factor_scores) < max(d1, d2)) {
    ...
    plot(data[, d1], data[, d2], col= data$cl, pch=16, axes = FALSE,
         ...
         xlab = NA,
         ylab = NA, main = "Factor Scores", cex = 2, cex.main = 3)
    legend("bottomleft", cex=1.8 ,legend = data$objects ,col=data$cl ,pch=16)
    ...
    d1_string <- paste(d1)
    d2_string <- paste(d2)
    text(0, margin+0.3, d2_string, cex= 2)
    text(margin+0.3, 0, d1_string, cex= 2)
  }
}
...
plot_pfs <- function (mymfa, X=1, d1 = 1, d2 = 2) {
  if (ncol(mymfa$common_factor_scores) < max(d1, d2)) {
    ...
    plot(data[, d1], data[, d2], col= data$cl, pch=16, axes = FALSE,
         ...
         xlab = NA,
         ylab = NA,
         main ="Partial Factor Scores" , cex = 2, cex.main = 3)
    legend("bottomleft", cex= 1.8 ,legend = data$objects ,col=data$cl ,pch=16)
    arrows(x0 = -1*margin-0.2, y0 = 0, x1 = margin+0.2, y1 = 0, length=0.05,angle=20,
           code = 2, lwd = 2)
    arrows(y0 = -1*margin-0.2, x0 = 0, y1 = margin+0.2, x1 = 0, length=0.05,angle=20,
...
plot_vl <- function (mymfa, X=1, d1 = 1, d2 = 2, loading_labels = NULL) {
  if (ncol(mymfa$common_factor_scores) < max(d1, d2)) {
    ...
    plot(data[, 1], data[, 2], col= data$cl, pch=16, axes = FALSE,
         ...
         xlab = NA,
         ylab = NA,
         main = "Variable Loadings" , cex = 2, cex.main = 3)
    if (!is.null(loading_labels)) {
      #decided to let user input data labels themselves since it is not included in csv file
      text(rescaled_loadings[,d1], rescaled_loadings[,d2], labels=loading_labels[1:nrow(rescaled_loadings)], cex= 2, pos=4)
    }
    else {
      legend("bottomleft", cex= 1.8,legend = data$objects ,col=data$cl ,pch=16)
    }
...
plot_pfs_vl <- function (mymfa, X=1, d1 = 1, d2 = 2, loading_labels = NULL) {
  if (ncol(mymfa$common_factor_scores) < max(d1, d2)) {
    ...
    plot(data[, d1], data[, d2], col= data$cl, pch=16, axes = FALSE,
         ...
         xlab = NA,
         ylab = NA,
         main = "Partial Factor Score and Variable Loadings" , cex = 2, cex.main = 2)
    legend("bottomleft", cex= 1.8 ,legend = data$objects ,col=data$cl ,pch=16)
    arrows(x0 = -1*margin-0.2, y0 = 0, x1 = margin+0.2, y1 = 0, length=0.05,angle=20,
           code = 2, lwd = 2)
    arrows(y0 = -1*margin-0.2, x0 = 0, y1 = margin+0.2, x1 = 0, length=0.05,angle=20,
    ...
    points(rescaled_loadings[,d1], rescaled_loadings[,d2], col = "grey", pch=12, cex = 2)
    if (!is.null(loading_labels)) {
      #decided to let user input data labels themselves since it is not included in csv file
      text(rescaled_loadings[,d1], rescaled_loadings[,d2], labels=loading_labels[1:nrow(rescaled_loadings)], cex= 2, pos=4)
    }
